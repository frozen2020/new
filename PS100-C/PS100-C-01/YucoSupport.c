...
void Initialize()
{
	...
	*XINT2CR = 0x8000;
	...
	ReadData();
		QSWEN =1; //tang
	// DIMAX, PULSERATE, LASSET. ENVSET, D1_SET, Time 
	if( (DIMAX < 0) || (DIMAX > 560) ) {
		DIMAX = 560;
		wrf = 1;
	}
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	if( (PULSERATE < 20000) || (PULSERATE > 150000) ) {     //原为100000
		PULSERATE = 20000;
		wrf = 1;
	}
	/* TEMPTBL1[0] = 15 Degrees C, 2738 = 38 Degrees C */
	if( (LASSET > 350) || (LASSET < 150) ) {
		LASSET = 350;
		wrf = 1;
	}
...
void dspfun()
{
	...
	else if (FUNCTION == 4)			// ENVIROMENTAL TEMPERATURE
		{
	 	message(msg5);
	 	PLACE = 25;
	 	num2ascii(ENVACT, 0);
...
void num2ascii(unsigned long number, short chr)
{
	...
	if(chr == 1) {
	    ...
	    number = (420 - number) / 4;           //500ns//y=(340-x)/3;Limit 40,310
	    if(number / 100)
	    HOLD[0] = '1';
	    else
	    HOLD[0] = ' ';
...
if( !(*ADCTRL2 & 0x1000) )    // MAKE SURE ADC READY
		...
		ADTEMP7[ADi] =	((((*RESULT7>>6)+(*RESULT15>>6))>>1) & 0x3FF);
   		ADTEMP0[20]+=ADTEMP0[ADi];
   		ADTEMP1[20]+=ADTEMP1[ADi];
		ADTEMP2[20]+=ADTEMP2[ADi];
		ADTEMP3[20]+=ADTEMP3[ADi];
		ADTEMP4[20]+=ADTEMP4[ADi];
		ADTEMP5[20]+=ADTEMP5[ADi];
		ADTEMP6[20]+=ADTEMP6[ADi];
	   	ADTEMP7[20]+=ADTEMP7[ADi];
   		}
		ADi++;
		if (ADi>=20)
		{ 
...
void tempctl(void)
{
	...
	*CMPR2 = (2000 - ((XTL2SET - 260) * 5.88));
	...
	//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    //LASACT=GETTEMP(T5);					//8/29 t3-->t4
	//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	OF1TEM=250;
	OF2TEM=GETTEMP(T6);
/*	if(LASACT < LASSET) *CMPR4 = 2000;
	else
		{
		error = (LASACT - LASSET);
		TEMP = (error * LHT_PG);
...
*/			
	...
	D1_INT = OLD_D1_INT + (Ki * (D1_ERROR ));  /* INTEGER TERM */
	...
	*T1CMPR = D1_OUT; //8/29
	...
	//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	LASACT=GETTEMP(T5);
	if( LASACT < 150) *T4CMPR = 0;
	if( LASACT > 300 ) *T4CMPR = 1500;
	
	/* Error is reference - current */
	FanLaser_ERROR = (( LASACT - LASSET)/10);		/* CALCULATE ERROR */
	FanLaser_PROP = FanLaser_ERROR * Kp;				/* PROPORTIONAL TERM*/				
	FanLaser_INT = OLD_FanLaser_INT + (Ki * (FanLaser_ERROR ));  /* INTEGER TERM */
	/* anti-windup test. NEED TO SET PROPER CUTOFF. */
	...
	*T4CMPR = FanLaser_OUT; //8/29
	TEMCFLAG=0;			/* reset timer 100 milliseconds*/
	//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
} 
...
void WriteData()
{short ENVSET2,i,DIMAX2,LASSET2,XTL2SET2;
 float D1_SET2;
...
while(i<3)
{
  if( PULSERATE2!=PULSERATE||PWMTime2!=PWMTime||ENVSET2!=ENVSET||DIMAX2!=DIMAX||XTL2SET2!=XTL2SET||LASSET2 != LASSET   )
 		{	DIMAX = DIMAX2  ;    
...
{	
	...
	if(((!FUNCTION)&&(clearWarning==0))||(REMLOC==1))
	{
		...
		if(DIMAX > 250)
		{
			if(DIODECUR < 12) EXTDCUR = DIODECUR;
//			if(DIODECUR < 12) EXTDCUR = (DIODECUR<<1);
				DIODECUR_ACT = EXTDCUR;
//				DIODECUR_ACT = EXTDCUR>>1;
				num2ascii(DIODECUR_ACT, 0);
		}
		else
		{	
			if(DIODECUR < 6 ) EXTDCUR = (DIODECUR<<2);
	...
	else
	{
		...
		//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			if (FUNCTION == 9)			// ENVIROMENTAL TEMPERATURE
		{
	 		PLACE = 25;
	 		num2ascii(OF1TEM, 0);
	 		PLACE = 35;
    	...
    	else if(FUNCTION == 4) num2ascii(ENVACT, 0);
   		else if(FUNCTION == 5) num2ascii(LASACT, 0);
